...
9. **Peristaltic pump control** — firmware-side hardening is complete in the separate `central_dosing_sys_peristaltic` repository (branch `harden-serial-protocol`) and verified on both Arduino MCUs on real hardware: drivers disabled by default, dose/runtime limits (`MAX_SINGLE_DOSE_ML`, `MAX_RUNTIME_MS`), a machine-readable `PING`/`STATUS`/`DOSE`/`STOP`/`STOPALL` protocol (see `docs/SERIAL_PROTOCOL.md` there). In `cds_control` itself there is still **no** Python serial client for it, and **no real peristaltic pumps are connected** — connecting it to recipe logic and the NiceGUI dashboard is deliberately a separate, not-yet-started step, and requires further safety validation (real calibration, verified pump assignment). A design-only plan for splitting the shared firmware into an `MCU_A_PH`/`MCU_B_EC` profile pair now exists at `docs/PERISTALTIC_PROFILE_PLAN.md` (no firmware/behavior changed by it).
