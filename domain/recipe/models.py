"""Typed recipe domain model: StoredRecipe, RecipePreview, RunOptions, RunConfigSnapshot.
...
These four types share one change reason (the recipe data model as a whole)
and RunConfigSnapshot.build() constructs directly from the other three, so
they stay together in one file rather than four near-empty ones.
"""
...
from .field_validators import validate_bool_field
from .limits import MAX_PROCESS_VOLUME_L, RecipeValidationError
from .validation import validate_recipe_field_types, validate_recipe_values
...
# a real int in __post_init__ below - see domain/recipe/field_validators.py
